...
for(var z=0; z<FD; z++) {
  for(var v=1; v<RB; v++) {
    ...
    fs_colors += `
      else if(layer==`+z+` && v==`+v+`u) ret = vec4(`+(rgb.r/255)+`, `+(rgb.g/255)+`, `+(rgb.b/255)+`, 1.);`;
  }
}
...
var fs_Color4Cell = `
  vec4 Color4Cell(uvec4 cell, int layer) {
    ...
    if(cell.b==0u) return ret;
    float sat = float(cell.b) / 255.;
    uint v;
...
var ShowProgram = createProgram(gl, CommonVertexShader, ShowFragmentShader, ["a_position", "u_fieldtexture", "u_canvas", "u_surface"]);

// SHOW MAIN ////////////////////////////////////////////////////////////////

function Show(single=0) {
