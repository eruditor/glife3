...
// auxiliary: floating-point math (Analysis)
var glmath_Format   = gl.RGBA;  //RGBA_INTEGER  // if EXT_color_buffer_float is not supported - use INT32
var glmath_Internal = gl.RGBA32F;  //RGBA32I
var glmath_Type     = gl.FLOAT;  //INT
...
function CreateTexture(width, height, depth=0, math=0) {
  ...
  else if(math) gl.texImage3D(gl_tex, 0, glmath_Internal, width, height, depth, 0, glmath_Format, glmath_Type, null);
...
function SetTexture(texture_num, texture, data, width, height, depth=0, math=0) {
  var gl_tex = depth==0 ? gl.TEXTURE_2D : gl.TEXTURE_3D;
  ...
  else if(math) gl.texImage3D(gl_tex, 0, glmath_Internal, width, height, depth, 0, glmath_Format, glmath_Type, data);
