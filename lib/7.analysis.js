...
var StatGraphFuncName = 'log';
     if(StatGraphFuncName=='log')   function StatGraphFunc(x) { return Math.log2(x); }
...
scnvs.width  = zoom * FW;    scnvs.style.width  = scnvs.width  + 'px';
...
function SaveRules(failed_at='') {  // only for Conway yet !!!
  if(Family=='conway') {
    ...
    q += (q?'&':'') + 'rules=' + encodeURIComponent(rules);
    
    q += (q?'&':'') + 'seed=' + encodeURIComponent(Rseed);  // !!! add Fseed?
...
function Stats(force=false) {
  ...
  //sstat += 'live cells = ' + rec[S1].ttl + '<br>';
