...
function CreateNavButtons() {
  ...
  <td>
    ...
    <input type=button value='ReField' style='width:70px;' onclick='if(confirm("ReField?")) { ReInitSeeds(true); Init(); Pause(-1); }'><br>
    <input type=button value='Restart' style='width:70px;' onclick='if(confirm("Restart?")) { ReInitSeeds();     Init(); Pause(-1); }'>
  </td><td>
    <input type=button value='ORGA' onclick='Pause(1); StatORGA(1);' title='Show ORGA calculus'><br>
    <input type=button value='ShowT0' onclick='Show(1);' title='Show current Field frame'><br>
  </td>
  </table>
  `;
